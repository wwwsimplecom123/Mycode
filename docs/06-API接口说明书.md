...
- `POST /api/email/analyze/quick`：浏览器探针快速检测接口，必须提供用户插件 Token；检测会写入持久化分析记录和任务队列。
- `GET /api/email/analyze/status/{analysis_id}`：查询本人提交的浏览器探针深度检测进度。

浏览器探针接口允许跨域调用，但强制使用与用户绑定的 `X-ShieldDome-Plugin-Token`，且禁止跨用户查询检测状态。插件检测会进入邮件事件、首页统计和审计日志。
